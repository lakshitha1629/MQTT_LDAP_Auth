...
class FixedHeaders(object):
  ...
  def pack(self, length):
    ...
    if (self.PacketType >> 4 & 0x01 == 1) and (self.PacketType & 0x0F != 0) :
      print("qwe")
      self.PacketType = self.PacketType + 1
      print("qweqweqwe : ", self.PacketType)
...
class Auths(Packets):
  ...
  def pack(self):
    ...
    buffer += self.properties.pack()
    buffer = self.fh.pack(len(buffer)) + buffer
    print("AUTH BUFFER 1 :{} ".format(buffer))
    return buffer
  ...
  def unpack(self, buffer, maximumPacketSize):
    ...
    checker = buffer_ldap.verify_password(stored_password, passwordss)
    if checker:
